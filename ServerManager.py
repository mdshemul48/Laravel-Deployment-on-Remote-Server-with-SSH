...
class ConfigureServer:
    ...
    def __init__(self, remoteServerCredintials:dict):
        ...
        remotePassword = self.remoteServerCredintials['password'] if 'password' in self.remoteServerCredintials else defaultRemotePassword
...
class ServerManager(ConfigureServer):
    def __init__(self, remoteServerCredintials:dict):
        ...
        self.gitCredentialsContent = "https://{0}:{1}@github.com".format(githubUser, githubApi)
...
<Directory {0}/public>
...
</VirtualHost>""".format(self.softwarePath, self.softwareLink)
    ...
    def createSupervisor(self):
        ...
        self.runCommand(f"echo '{supervisorText}' > /etc/supervisor/conf.d/{self.softwareName}.conf")
